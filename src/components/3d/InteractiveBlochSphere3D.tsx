...
function seededRandom(seed: number): number {
  ...
  const { particlePositions, particlePhases, particleBaseRadii, particleDensities } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const u = seededRandom(i * 5 + 1);
      const v = seededRandom(i * 5 + 2);
      const t = Math.acos(2 * u - 1);
      const p = 2 * Math.PI * v;
      // Multi-layered depth distribution
      const layerOffset = (seededRandom(i * 5 + 3) - 0.5) * 0.9;
      ...
      phases[i] = seededRandom(i * 5 + 4) * Math.PI * 2;
      radii[i] = r;
      densities[i] = seededRandom(i * 5 + 5);
    }
    return {
