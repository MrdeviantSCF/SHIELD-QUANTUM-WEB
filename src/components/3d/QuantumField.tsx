...
function BlochSphereCore() {
  ...
  const vectorLineRef = useRef<THREE.Line>(null);
  ...
  useFrame((state) => {
    ...
    if (vectorLineRef.current) {
      const pos = vectorLineRef.current.geometry.attributes.position.array as Float32Array;
      pos[3] = x;
      pos[4] = y;
      pos[5] = z;
      vectorLineRef.current.geometry.attributes.position.needsUpdate = true;
    }
  ...
  return (
    <group ref={groupRef} position={[0, 0.4, 0]}>
      ...
      <primitive
        ...
        object={
          new THREE.Line(
            new THREE.BufferGeometry().setAttribute('position', new THREE.BufferAttribute(new Float32Array([0, 0, 0, 0, 2.4, 0]), 3)),
            new THREE.LineBasicMaterial({ color: '#00d4ff', linewidth: 2, transparent: true, opacity: 0.85 })
          )
        }
      />
...
function EntangledQubitPairs() {
  ...
  const pairs = useMemo(() => {
    return Array.from({ length: pairCount }).map((_, i) => ({
      ...
      tilt: (seededRandom(i * 3 + 1) - 0.5) * 0.5,
...
function QuantumCloudParticles() {
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      const r = 12 * Math.cbrt(seededRandom(i * 4 + 1));
